...
public class Itest {
    public static void main(String[] args) {
        ...
        // 包下面的类
        Set<Class<?>> classes = getClasses("com.demo");
        if (classes != null && classes.size() > 0) {
            classes.stream().filter(clazz -> !clazz.getName().contains("$")).filter(clazz -> !clazz.isInterface()).map(clazz -> clazz.getName()).forEach(System.out::println);
        }
